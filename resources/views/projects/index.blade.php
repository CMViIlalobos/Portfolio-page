    ...
    @if($categories->count() > 0)
        <nav class="mt-10 flex flex-wrap gap-2" data-reveal style="--delay: 80ms" aria-label="Filter projects by category">
            <a
                ...
                class="tag {{ request()->routeIs('projects.index') ? 'tag-brand' : '' }}"
    ...
    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        ...
        @empty
            <div class="surface p-12 text-center lg:col-span-3">
                <p class="font-display text-xl font-bold">Nothing published here yet.</p>
                <p class="mt-2 text-sm text-[rgb(var(--muted))]">New case studies are added as projects ship — check back soon.</p>
            </div>
        @endforelse
    </div>
